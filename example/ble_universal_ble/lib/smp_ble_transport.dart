...
/// A complete [SmpTransport] over BLE GATT, built on `universal_ble`.
///
/// This is **example code** — `universal_ble` (and Flutter) are dependencies of
/// this example package only, never of `mcumgr_dart` itself. It is the same
/// transport ProtoCentral ships in production, lightly trimmed for clarity.
///
/// SMP is carried on the Nordic SMP GATT service via a single characteristic
/// used in **both** directions — write-without-response for requests, notify
/// for responses:
///   - Service        `8D53DC1D-1DB7-4CD3-868B-8A527460AA84`
///   - Characteristic `DA2E7828-FBCE-4E01-AE9E-261174997C48`
///
/// A device is "SMP-enabled" iff [connect] finds that service; otherwise it
/// throws [SmpTransportException] so callers can gate on it.
class SmpBleTransport implements SmpTransport {
  ...
  Future<void> connect() async {
    ...
    try {
      ...
      // Watch for drops only AFTER connect succeeds.
      ...
      // maxWriteLength = MTU − 3 (ATT header) bounds outbound frame chunking for
      // image upload. Request the max so DFU chunks are as large as the stack
      // allows; stays null if unknown (ImgMgmt/FsMgmt use a safe default then).
      try {
      ...
      // Pipe raw notifications into our broadcast stream, then enable them.
      _valueSub =
  ...
  /// Re-query the negotiated MTU. On macOS/iOS the MTU exchange completes
  /// shortly *after* connect, so call this once the link has settled (or right
  /// before a large transfer) to pick up the real value.
  ...
  /// Dispose the broadcast controllers once the transport is discarded for good.
